...
class Expenses_ao_model extends App_Model
{
        ...
        foreach (['file', 'comprovante'] as $field) {
            if (isset($files[$field]) && $files[$field]['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($files[$field]['tmp_name'], $filepath)) {
                    ...
                    $this->db->update(db_prefix() . 'expenses_ao', [$field => $file_url]);
                }
            }
        }
    }
    ...
    public function upload_file($expense_id, $file, $field_name)
    {
        ...
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            $this->db->where('id', $expense_id);
            $this->db->update(db_prefix() . 'expenses_ao', [$field_name => $filename]);
    ...
    public function get_child_expenses($id)
    {
        ...
        $expenses = $this->db->get(db_prefix() . 'expenses_ao')->result_array();
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'expenses_ao', $data);
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('recurring_from', $id);
            $this->db->update(db_prefix() . 'expenses_ao', ['recurring_from' => null]);
    ...
    public function convert_to_invoice($id, $draft_invoice = false, $params = [])
    {
        ...
        if ($invoiceid) {
            ...
            $this->db->update(db_prefix() . 'expenses_ao', [
                'invoiceid' => $invoiceid,
            ]);
                ...
                foreach ($cfExpenses as $field) {
                    ...
                    if (isset($tmpSlug[1])) {
                        ...
                        if (count($cfTransfer) == 1 && ((similarity($field['name'], $cfTransfer[0]['name']) * 100) >= CUSTOM_FIELD_TRANSFER_SIMILARITY)) {
                            $value = get_custom_field_value($id, $field['id'], 'expenses_ao', false);
                            if ($value == '') {
                                continue;
                            }
    ...
    public function copy($id)
    {
        ...
        if ($insert_id) {
            ...
            $custom_fields = get_custom_fields('expenses_ao');
    ...
    {
        if (is_reference_in_table('category', db_prefix() . 'expenses_ao', $id)) {
            return [
                'referenced' => true,
            ];
        }
