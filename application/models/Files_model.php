...
public function delete_files_by_folder($folder_id)
{
        ...
        try {
            ...
            if (empty($items_in_dir)) {
                ...
                return [
                    ...
                    'http_status' => REST_Controller::HTTP_CONFLICT
                ];
            }
        } catch (Exception $e) {
            return [
    ...
    private function prepare_chart_data_structure($categories_labels) {
        ...
        foreach ($file_categories as $category_name) {
            $chart_series_data[$category_name] = [
                'name' => $category_name,
                'data' => array_fill(0, count($categories_labels), 0),
                'dataValueQuantity' => array_fill(0, count($categories_labels), 0)
            ];
        }
    ...
    public function get_weekly_stats() {
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            if ($year_num < $current_year && $week_num > $current_week) {
                $adjusted_week_num = $week_num + 100;
            }

            $recent_weeks_map[$adjusted_week_num] = 4 - $i;
        }
        ksort($recent_weeks_map);

        $categories_labels_ordered = [];
        $index_counter = 0;
        foreach ($recent_weeks_map as $adj_week => $original_index) {
            $date = new DateTime();
            $date->setISODate($current_year, $adj_week % 100);
        ...
        foreach ($chart_series_data as $cat_name => &$series) {
            foreach ($categories_labels as $index => $label) {
    ...
    public function get_monthly_stats() {
        ...
        $temp_monthly_data_size = [];
        $temp_monthly_data_quantity = [];
    ...
    public function get_yearly_stats() {
        ...
        $temp_yearly_data_size = [];
        $temp_yearly_data_quantity = [];
