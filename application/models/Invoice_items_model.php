    ...
    public function bulk_create_products($products, $warehouse_id, $edit_product_id = null)
    {
        ...
        try {
            foreach ($products as $index => $product_data) {
                ...
                if (
                    !isset($cleaned_product_data['cubage']) &&
                ...
                if ($product_id) {
                    ...
                    if ($input_primary_index !== null && isset($input_images_array_b64[$input_primary_index]) && !empty($input_images_array_b64[$input_primary_index])) {
                        ...
                        if (preg_match('/^data:image\/(\w+);base64,(.*)$/s', $b64, $m)) {
                            $img_type_h = strtolower($m[1]);
                            $img_data_h = base64_decode($m[2], true);
                        } else {
                            $img_data_h = base64_decode($b64, true);
                        }
                        if ($img_data_h && file_exists($upload_dir)) {
                            $fn_h = uniqid() . '.' . $img_type_h;
                            $fp_h = $upload_dir . $fn_h;
                            if (file_put_contents($fp_h, $img_data_h)) {
                                $s_h = base_url();
                                $r_h = str_replace('./', '', $fp_h);
                                $url = rtrim($s_h, '/') . '/' . $r_h;
                            }
                        }
                        if ($url) {
                    ...
                    if (!$uploaded_primary_url && !empty($images_for_other_slots_b64)) {
                        foreach ($images_for_other_slots_b64 as $key => $b64_val) {
                            if (!empty($b64_val)) {
                                ...
                                $img_data_h = null;
                                $img_type_h = 'jpg';
                                if (preg_match('/^data:image\/(\w+);base64,(.*)$/s', $b64_val, $m)) {
                                    $img_type_h = strtolower($m[1]);
                                    $img_data_h = base64_decode($m[2], true);
                                } else {
                                    $img_data_h = base64_decode($b64_val, true);
                                }
                                if ($img_data_h && file_exists($upload_dir)) {
                                    $fn_h = uniqid() . '.' . $img_type_h;
                                    $fp_h = $upload_dir . $fn_h;
                    ...
                    if (!$uploaded_primary_url && !empty($input_image_singular_b64)) {
                        ...
                        $img_data_h = null;
                        $img_type_h = 'jpg';
                        if (preg_match('/^data:image\/(\w+);base64,(.*)$/s', $input_image_singular_b64, $m)) {
                            $img_type_h = strtolower($m[1]);
                            $img_data_h = base64_decode($m[2], true);
                        } else {
                            $img_data_h = base64_decode($input_image_singular_b64, true);
                        }
                        if ($img_data_h && file_exists($upload_dir)) {
                            $fn_h = uniqid() . '.' . $img_type_h;
                            $fp_h = $upload_dir . $fn_h;
                    ...
                    foreach ($images_for_other_slots_b64 as $b64) {
                        if (empty($b64))
                            continue;
                        if ($current_other_idx >= count($other_image_fields_map))
                            break;
