...
class Staff_model extends App_Model
{
    ...
    // public function get_api2($id = '', $page = 1, $limit = 10, $search = '', $sortField = 'staffid', $sortOrder = 'ASC', $type = 'franchisees', $warehouse_id = 0, $franqueado_id = 0)
    // {
    //     $this->load->model("roles_model");
    ...
    public function get_api2($id = '', $page = 1, $limit = 10, $search = '', $sortField = 'staffid', $sortOrder = 'ASC', $type = 'franchisees', $warehouse_id = 0)
    {
        $this->load->model("roles_model");
        ...
        if (!is_numeric($id)) {
            ...
            // Aplicar filtro pelo warehouse_id (agora descomentado e funcionando)
            if (!empty($warehouse_id)) {
                $this->db->where(db_prefix() . 'staff.warehouse_id', $warehouse_id);
            }

            if (!empty($search)) {
                $this->db->group_start();
                $this->db->like(db_prefix() . 'staff.firstname', $search);
                $this->db->or_like(db_prefix() . 'staff.lastname', $search);
                $this->db->or_like(db_prefix() . 'staff.email', $search);
                $this->db->or_like(db_prefix() . 'staff.phonenumber', $search);
                $this->db->or_like(db_prefix() . 'staff.vat', $search);
                $this->db->group_end();
            }
