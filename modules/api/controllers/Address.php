...
class Address extends REST_Controller {
     ...
     *     }
     */
    public function data_get($userid, $id = '') {
        ...
        $data = $this->Address_model->get_api($userid, $id, $page, $limit, $search, $sortField, $sortOrder);
    ...
    /**
     * @api {post} api/customers Add New Address
     * @apiName PostAddress
     * @apiGroup Address
     *
     * @apiHeader {String} Authorization Basic Access Authentication token.
     *
     * @apiParam {String} company               Mandatory Address company.
     * @apiParam {String} [vat]                 Optional Vat.
     * @apiParam {String} [phonenumber]         Optional Address Phone.
     * @apiParam {String} [website]             Optional Address Website.
     * @apiParam {Number[]} [groups_in]         Optional Address groups.
     * @apiParam {String} [default_language]    Optional Address Default Language.
     * @apiParam {String} [default_currency]    Optional default currency.
     * @apiParam {String} [address]             Optional Address address.
     * @apiParam {String} [city]                Optional Address City.
     * @apiParam {String} [state]               Optional Address state.
     * @apiParam {String} [zip]                 Optional Zip Code.
     * @apiParam {String} [partnership_type]    Optional Address partnership type.
     ...
     * @apiSuccess {String} message Address add successful.
     *
     * @apiSuccessExample Success-Response:
     *     HTTP/1.1 200 OK
     *     {
     ...
     *
     * @apiError {Boolean} status Request status.
     * @apiError {String} message Address add fail.
     *
     * @apiErrorExample Error-Response:
     *     HTTP/1.1 404 Not Found
     *     {
     ...
     *
     */
    public function data_post($userid) {

        $this->load->model('Address_model');
                    $insert_data = $_POST = json_decode($this->security->xss_clean(file_get_contents("php://input")), true);
                    $insert_data['userid'] = $userid;

        // form validation
        $this->form_validation->set_rules('address', 'address', 'trim|required|max_length[600]', array('is_unique' => 'This %s already exists please enter another Company'));
        if ($this->form_validation->run() == FALSE) {
            // form validation error
            $message = array('status' => FALSE, 'error' => $this->form_validation->error_array(), 'message' => validation_errors());
            $this->response($message, REST_Controller::HTTP_NOT_FOUND);
        } else {
            $output = $this->Address_model->add($insert_data);
            if ($output > 0 && !empty($output)) {
                // success
                $message = array('status' => TRUE, 'message' => 'Address add successful.', 'data' =>$output );
                $this->response($message, REST_Controller::HTTP_OK);
            } else {
                // error
                $message = array('status' => FALSE, 'message' => 'Address add fail.');
                $this->response($message, REST_Controller::HTTP_NOT_FOUND);
            }
        }
    }
    ...
    /**
     * @api {delete} api/delete/customers/:id Delete a Address
     ...
     * @apiSuccess {String} message Address Delete Successful.
     *
     * @apiSuccessExample Success-Response:
     *     HTTP/1.1 200 OK
     *     {
     ...
     *
     * @apiError {Boolean} status Request status.
     * @apiError {String} message Address Delete Fail.
     *
     * @apiErrorExample Error-Response:
     *     HTTP/1.1 404 Not Found
     *     {
    ...
    public function data_delete($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            // delete data
            $this->load->model('Address_model');
            $output = $this->Address_model->delete($id);
            if ($output === TRUE) {
                // success
                $message = array('status' => TRUE, 'message' => 'Address Delete Successful.');
                $this->response($message, REST_Controller::HTTP_OK);
            } else {
                // error
                $message = array('status' => FALSE, 'message' => 'Address Delete Fail.');
                $this->response($message, REST_Controller::HTTP_NOT_FOUND);
            }
        }
    }
    ...
    /**
     * @api {put} api/customers/:id Update a Address
     * @apiName PutAddress
     * @apiGroup Address
     *
     * @apiHeader {String} Authorization Basic Access Authentication token.
     ...
     *
     * @apiSuccess {Boolean} status Request status.
     * @apiSuccess {String} message Address Update Successful.
     *
     * @apiSuccessExample Success-Response:
     *     HTTP/1.1 200 OK
     *     {
     ...
     *
     * @apiError {Boolean} status Request status.
     * @apiError {String} message Address Update Fail.
     *
     * @apiErrorExample Error-Response:
     *     HTTP/1.1 404 Not Found
     *     {
    ...
    public function data_put($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            // update data
            $output = $this->Address_model->update($update_data, $id);
            if ($output > 0 && !empty($output)) {
                // success
                $message = array('status' => TRUE, 'message' => 'Addresss Update Successful.', 'data' =>$this->Address_model->get($id));
                $this->response($message, REST_Controller::HTTP_OK);
            } else {
                // error
                $message = array('status' => FALSE, 'message' => 'Addresss Update Fail.');
                $this->response($message, REST_Controller::HTTP_NOT_FOUND);
            }
        }
    }
