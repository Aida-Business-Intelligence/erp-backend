...
public function favorite_put($id)
{
    ...
    $current_is_favorite = (bool) $file['is_favorite'];
    $new_is_favorite     = (bool) $input['is_favorite'];
    ...
    $result = $this->Files_model->update_file_favorite($id, $new_is_favorite);
    if ($result) {
        $this->response([
            'status' => true,
    ...
    public function delete_delete($id)
{
    ...
    if ($result) {
        ...
        if (is_dir($file_directory) && $file_directory !== $base_upload_dir) {
            $items_in_dir = array_diff(scandir($file_directory), array('.', '..'));
...
public function bulk_delete_by_folder_delete($folder_id)
{
    ...
    public function chart_get()
    {
        \modules\api\core\Apiinit::the_da_vinci_code('api');
        $chart_data = [];
        $weekly_stats = $this->Files_model->get_weekly_stats();
        $chart_data[] = [
            'name' => 'Semanal',
            'categories' => $weekly_stats['categories'],
            'data' => $weekly_stats['data'],
        ];
        $monthly_stats = $this->Files_model->get_monthly_stats();
        $chart_data[] = [
            'name' => 'Mensal',
            'categories' => $monthly_stats['categories'],
            'data' => $monthly_stats['data'],
        ];
        $yearly_stats = $this->Files_model->get_yearly_stats();
        $chart_data[] = [
            'name' => 'Anual',
