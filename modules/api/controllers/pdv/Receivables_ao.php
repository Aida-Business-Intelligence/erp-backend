...
class Receivables_ao extends REST_Controller
{
    ...
    public function get_get($id = null)
    {
        ...
        try {
            $receivable = $this->Receivables_ao_model->get_receivable_by_id($id);
    ...
    private function receive_multiple_installments($receivable_id, $installment_numbers, $data)
    {
        try {
            ...
            if ($summary['parcelas_pendentes'] == 0) {
                ...
                $this->load->model('Receivables_ao_model');
                $this->Receivables_ao_model->update([
                    'status' => 'received',
                ], $receivable_id);
            }
    ...
    {
        try {
            $paymentModes = $this->Receivables_ao_model->get_payment_modes();
    ...
    public function clients_get()
    {
        try {
            ...
            if ($type === 'franchisees') {
                $clients = $this->Receivables_ao_model->get_franchisees($warehouse_id, $search, $limit, $page);
            } else {
                $clients = $this->Receivables_ao_model->get_clients($warehouse_id, $search, $limit, $page);
            }
    ...
    public function create_post()
    {
        ...
        if ($is_multipart && isset($_FILES['receivables_document']) && $_FILES['receivables_document']['error'] === UPLOAD_ERR_OK && $_FILES['receivables_document']['size'] > 0) {
            ...
            $blobName = 'uploads_erp/' . getenv('NEXT_PUBLIC_CLIENT_MASTER_ID') . '/' . $input['warehouse_id'] . '/receivables_ao/documents/' . $unique_filename;
        ...
        $this->load->model('Receivables_ao_model');
        $id = $this->Receivables_ao_model->add($data);
    ...
    public function update_post($id = null)
    {
        ...
        $this->load->model('Receivables_ao_model');
        $current = $this->Receivables_ao_model->get_receivable_by_id($id);
        ...
        if ($is_multipart && isset($_FILES['receivables_document']) && $_FILES['receivables_document']['error'] === UPLOAD_ERR_OK && $_FILES['receivables_document']['size'] > 0) {
            ...
            $unique_filename = 'receivable_' . time() . '_' . uniqid() . '.' . $file_extension;
            $blobName = 'uploads_erp/' . getenv('NEXT_PUBLIC_CLIENT_MASTER_ID') . '/' . $warehouse_id . '/receivables_ao/documents/' . $unique_filename;
        ...
        $data = [
            ...
            'juros_apartir' => $input['juros_apartir'] ?? 1,
            'total_parcelado' => $input['total_parcelado'] ?? $input['amount'],
            'nota_tipo_ao' => $input['nota_tipo_ao'] ?? null,
            // 'tipo_juros' => $input['tipo_juros'] ?? 'simples', // continua fora do principal
        ];
        ...
        $this->load->model('Receivables_ao_model');
        $success = $this->Receivables_ao_model->update($data, $id);
    ...
    public function delete_post()
    {
        ...
        $success = $this->Receivables_ao_model->delete($id);
    ...
    public function list_by_day_post()
    {
        ...
        $result = $this->Receivables_ao_model->get_receivables_by_day($params);
    ...
    private function upload_voucher($receivable_id, $file)
    {
        ...
        $receivable = $this->Receivables_ao_model->get_receivable_by_id($receivable_id);
        $warehouse_id = $receivable ? $receivable->warehouse_id : 0;

        $unique_filename = 'voucher_' . $receivable_id . '_' . time() . '_' . uniqid() . '.' . $file_extension;
        $blobName = 'uploads_erp/' . getenv('NEXT_PUBLIC_CLIENT_MASTER_ID') . '/' . $warehouse_id . '/receivables_ao/vouchers/' . $receivable_id . '/' . $unique_filename;
