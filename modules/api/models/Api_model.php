...
class Api_model extends App_Model {
    ...
    public function get_table($name, $id, $page = 1, $limit = 10, $search='', $sortField='id', $sortOrder='ASC') {
        ...
        switch ($name) {
            ...
            case 'clients':
                ...
                return $this->Clients_model->get_api($id, array(), $page, $limit, $search, $sortField, $sortOrder);
                break;
            ...
            case 'invoices':
                ...
                return $this->Invoices_model->get_api($id, array(), $page, $limit);
            ...
            case 'items':
                ...
                return $this->invoice_items_model->get_api($id, array(), $page, $limit);
                break;
    ...
    public function search($type, $key, $page = 1, $limit = 50) {
        \modules\api\core\Apiinit::the_da_vinci_code('api');

        return $this->get_relation_data_api($type, $key, $page, $limit);
    }

    public function search_api($type, $key, $page = 1, $limit = 50) {
        \modules\api\core\Apiinit::the_da_vinci_code('api');
        return $this->get_relation_data_api_search($type, $key, $page, $limit);
    }

    public function get_relation_data_api_search($type, $search = '', $page = 1, $limit = 50) {
        \modules\api\core\Apiinit::the_da_vinci_code('api');
        $q = trim(urldecode($search));
        $data = [];
        ...
        if ('customer' == $type || 'customers' == $type) {
            ...
            if ($q) {
              ...
              //  $where_clients .= 'company LIKE "%' . $q . '%" OR CONCAT(firstname, " ", lastname) LIKE "%' . $q . '%" OR email LIKE "%' . $q . '%"';
                ...
                foreach ($fields as $key => $value) {
                    ...
                    $where_clients .= ' OR ctable_' . $key . '.value LIKE "%' . $this->db->escape_like_str($q) . '%"';
                }
                $where_clients .= ')';
                 * 
                 */
            ...
            // Load clients_model to perform the search
            $this->load->model('clients_model');

            // Get customers with limit and offset
            $data['customers'] = $this->clients_model->get_api('', $where_clients, $page, $limit);

      
            // Count total number of customers matching the search criteria for pagination
            $total_query = $this->db->query('SELECT COUNT(*) as total FROM tblclients WHERE active=1 AND (' . $where_clients . ')');
            $total_result = $total_query->row();
            $data['total'] = $total_result ? (int) $total_result->total : 0; // Assign total count
        } elseif ('contacts' == $type) {
            // Similar logic for handling 'contacts' would go here...
            // Make sure to set $data['customers'] and $data['total'] just like above
            // Example (not implemented here for brevity):
            // $data['customers'] = $this->contacts_model->get(...);
            // $data['total'] = ...;
        } elseif ('ticket' == $type) {
            $search = $this->_search_tickets($q, $offset, true, $limit);
            $data['customers'] = $search['result'];
            $data['total'] = count($data['customers']);
        } elseif ('lead' == $type || 'leads' == $type) {
            $search = $this->_search_leads($q, $offset, ['junk' => 0], true, $limit);
            $data['customers'] = $search['result'];
            $data['total'] = count($data['customers']);
        } elseif ('invoice' == $type || 'invoices' == $type) {
            $search = $this->_search_invoices($q, $offset, [], true, $limit);
            $data['customers'] = $search['result'];
            $data['total'] = count($data['customers']);
            ...
            // Apply limit and offset for items query
            $items = $this->db->get(db_prefix() . 'items', $limit, $offset)->result_array();
            ...
            $data['customers'] = $items;  // the key here might need to be renamed for clarity
            $data['total'] = $this->db->count_all_results(db_prefix() . 'items'); // Adjust to get item count
        } elseif ('project' == $type) {
            // Handle 'project' type similarly
        } elseif ('staff' == $type) {
            // Handle 'staff' type similarly
        } elseif ('tasks' == $type) {
            // Handle 'tasks' type similarly
        } elseif ('payments' == $type) {
            // Handle 'payments' type similarly
        } elseif ('proposals' == $type) {
            // Handle 'proposals' type similarly
        } elseif ('estimates' == $type) {
            // Handle 'estimates' type similarly
        } elseif ('expenses' == $type) {
            // Handle 'expenses' type similarly
        } elseif ('creditnotes' == $type) {
            // Handle 'creditnotes' type similarly
        } elseif ('milestones' == $type) {
            $where_milestones = '';
            if ($q) {
                $where_milestones .= '(name LIKE "%' . $this->db->escape_like_str($q) . '%" OR id LIKE "%' . $this->db->escape_like_str($q) . '%" )';
            }

            // Get milestones with pagination
            $data['customers'] = $this->get_milestones_api('', $where_milestones, $limit, $offset); // Adjust for pagination
            $data['total'] = $this->db->query('SELECT COUNT(*) as total FROM milestones WHERE ' . $where_milestones)->row()->total; // Adjust to get the total count of milestones
        }
    ...
    public function get_relation_data_api2($type, $search = '', $page = 1, $limit = 50) {
        $offset = ($page - 1) * $limit; // Calculate offset
        ...
        if ('customer' == $type || 'customers' == $type) {
            ...
            $this->load->model('clients_model');
            $data = $this->clients_model->get('', $where_clients, $limit, $offset);
