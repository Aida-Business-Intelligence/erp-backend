<?php
class Rednumber_Marketing_CRM_Frontend_Perfex_EventON{
	private static $add_on ="perfex"; 
	private static $form ="eventon"; 
	private $datas_submits= array(); 
	function __construct(){
		add_filter('evotx_beforesend_tix_email_data', array($this,'submit_form'),10,2);
	}
	...
	function submit_form($data, $order_id){
		$form_id = "eventon_ticket";
		$shortcode = new PDF_Eventon_Shortcodes();
		...
		if( is_array($datas) && count($datas) > 0 && $options["api"] != ""){
			$this->add_submit($datas,$token_woo);
		}
	}
...
new Rednumber_Marketing_CRM_Frontend_Perfex_EventON;