<?php
class Rednumber_Marketing_CRM_Frontend_Perfex_Fluent_Forms{
	private static $add_on ="perfex"; 
	private static $form ="fluent_forms"; 
	private $datas_submits= array(); 
	function __construct(){
		add_action('fluentform_integration_notify_notifications', array($this,'submit_form'),10,4);
	...
	function submit_form($feed, $formData, $entry, $form ){
		$form_id = $form->id;
		$entry = (array) $entry;
		$submission = array();
		foreach( $entry["user_inputs"] as $k=>$v) {
			$submission["inputs.".$k]= $v;
		}
...
new Rednumber_Marketing_CRM_Frontend_Perfex_Fluent_Forms;