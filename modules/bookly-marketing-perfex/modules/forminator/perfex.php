<?php
class Rednumber_Marketing_CRM_Frontend_Perfex_Forminator_Forms{
	private static $add_on ="perfex"; 
	private static $form ="forminator"; 
	private $datas_submits= array(); 
	function __construct(){
		add_action('forminator_form_after_save_entry', array($this,'submit_form'),10,2);
	...
	function submit_form($form_id, $response  ){
		...
		$submission = array();
		foreach( $entry["meta_data"] as $k=>$v) {
			$submission["{".$k."}"]= $v["value"];
		}
...
new Rednumber_Marketing_CRM_Frontend_Perfex_Forminator_Forms;